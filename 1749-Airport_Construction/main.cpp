...
double prolong_and_find_longest_segment(const int i1, const int i2, const std::vector<point> points) {
    ...
    {
        if(i != i1  && i != i2 && ((i + 1) % points.size()) != i1 && ((i + 1) % points.size()) != i2) {
            cp1 = points[i];
    ...
    // get segments inside the polygone from the finded intersection points
    std::vector<segment> segs;
    
    // return longest distance
    double best_distance = 0;
    for (segment seg : segs){
        best_distance = max(best_distance, seg.distance);
    }
